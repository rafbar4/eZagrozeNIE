...
ROUTE_SIMPLIFICATION_TOLERANCE = 0.0025  # Simplify route geometry
...
def calculate_probability(accidents_df):
    ...
    final_dataframe = accidents_df[['lon', 'unix_time']].copy()

    # tworzenie kolumny "pora dnia":
    final_dataframe['time_of_day'] = [
        '0-6' if datetime.datetime.fromtimestamp(i).hour in range(0, 6)
        else '6-9' if datetime.datetime.fromtimestamp(i).hour in range(6, 9)
        else '9-14' if datetime.datetime.fromtimestamp(i).hour in range(9, 14)
        else '14-18' if datetime.datetime.fromtimestamp(i).hour in range(14, 18)
        else '18-24'
        for i in final_dataframe['unix_time']
    ]

    # kolumna "dzień":
    final_dataframe['day'] = [
        datetime.datetime.fromtimestamp(i).day
        for i in final_dataframe['unix_time']
    ]

    # kolumna "miesiąc":
    final_dataframe['month'] = [
        datetime.datetime.fromtimestamp(i).month
        for i in final_dataframe['unix_time']
    ]

    # kolumna "rok":
    final_dataframe['year'] = [
        datetime.datetime.fromtimestamp(i).year
        for i in final_dataframe['unix_time']
    ]
    ...
    for i in range(len(month_lengths)):
        final_dataframe = pd.concat([
            final_dataframe,
            pd.DataFrame({
                'lon': [None] * month_lengths[i] * 5,
                'time_of_day':
                    ['0-6'] * month_lengths[i] +
                    ['6-9'] * month_lengths[i] +
                    ['9-14'] * month_lengths[i] +
                    ['14-18'] * month_lengths[i] +
                    ['18-24'] * month_lengths[i],
                'day': [x for x in range(1, month_lengths[i] + 1)] * 5,
                'month': [i + 1] * month_lengths[i] * 5,
                'year': [2024] * month_lengths[i] * 5
            })
        ], ignore_index=True)

    # usuwanie duplikatów:
    final_dataframe.drop_duplicates(
        subset=['time_of_day', 'day', 'month', 'year'],
        keep='first',
        inplace=True,
        ignore_index=True
    )

    # dychotomizacja zmiennej wypadkowej:
    ...
    final_dataframe['weekday'] = [
        datetime.date(
            int(final_dataframe['year'][i]),
            int(final_dataframe['month'][i]),
            int(final_dataframe['day'][i])
        ).weekday() + 1
        for i in range(len(final_dataframe))
    ]

    # dni i lata nie będą brane pod uwagę:
    final_dataframe.drop(['day', 'year'], axis='columns', inplace=True)

    # dummy variables do regresji logistycznej:
    final_dataframe = pd.get_dummies(
        final_dataframe,
        columns=['time_of_day', 'weekday', 'month']
    )

    # interakcje (uwaga: zależne od kolejności kolumn)
    for i in range(1, 6):
        for j in range(6, 13):
            final_dataframe[
                final_dataframe.columns[j] + '*' + final_dataframe.columns[i]
            ] = final_dataframe[final_dataframe.columns[i]] * final_dataframe[final_dataframe.columns[j]]

    # estymacja modelu:
    model = LogisticRegression(max_iter=1000)
    model.fit(
        final_dataframe.drop('accidents', axis='columns'),
        final_dataframe['accidents']
    )
    ...
    time_of_day = (
        '0-6' if now.hour in range(0, 6) else
        '6-9' if now.hour in range(6, 9) else
        '9-14' if now.hour in range(9, 14) else
        '14-18' if now.hour in range(14, 18) else
        '18-24'
    )

    pred_dataframe = pd.DataFrame(columns=final_dataframe.columns[1:])

    # jednowierszowy dataframe predykcyjny:
    for col in pred_dataframe.columns:
        pred_dataframe[col] = [0]
